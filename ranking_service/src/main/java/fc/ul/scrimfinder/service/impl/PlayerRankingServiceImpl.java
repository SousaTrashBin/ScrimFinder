...
import fc.ul.scrimfinder.rest.client.MatchHistoryClient;
...
public class PlayerRankingServiceImpl implements PlayerRankingService {
    ...
    @Inject
    @RestClient
    MatchHistoryClient matchHistoryClient;
    ...
    public List<PlayerRankingDTO> getPlayerRanking(Long playerId, Optional<Long> queueId) {
        ...
        if (queueId.isPresent()) {
            QueueEntity queue = queueRepository.findByIdOptional(queueId.get())
                    .orElseThrow(() -> new QueueNotFoundException("Queue not found"));
            return playerRankingRepository.findByPlayerAndQueue(player, queue)
                    .map(ranking -> List.of(playerRankingMapper.toDTO(ranking)))
                    .orElse(Collections.emptyList());
        }

        return playerRankingRepository.find("player", player).list()
                .stream().map(playerRankingMapper::toDTO).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public Map<Long, PlayerRankingDTO> processMatchResults(MatchResultRequest request) {
        QueueEntity queue = queueRepository.findByIdOptional(request.queueId())
                .orElseThrow(() -> new QueueNotFoundException("Queue not found"));
        ...
        for (Map.Entry<Long, MatchResultRequest.PlayerDelta> entry : request.playerDeltas().entrySet()) {
        ...
        matchHistoryClient.saveBatchMMRGains(request.gameId(), finalDeltas);

        return results;
    }

    private void updatePlayerMMRInternal(Long playerId, QueueEntity queue, int delta, boolean isWin,
                                         Map<Long, PlayerRankingDTO> results, Map<Long, Integer> playerDeltas) {
        Player player = playerRepository.findByIdOptional(playerId)
                .orElseThrow(() -> new PlayerNotFoundException("Player " + playerId + " not found"));

        PlayerRanking ranking = playerRankingRepository.findByPlayerAndQueue(player, queue)
                .orElseThrow(() -> new MMRAlreadyExistsException("Ranking not found for player " + playerId + " in this queue"));
    ...
    public PaginatedResponseDTO<PlayerRankingDTO> getQueueLeaderboard(int page, int size, Optional<Long> queueId) {
        ...
        if (queueId.isPresent()) {
            QueueEntity queue = queueRepository.findByIdOptional(queueId.get())
                    .orElseThrow(() -> new QueueNotFoundException("Queue not found"));
            query = playerRankingRepository.find("queue", queue);
        } else {
            query = playerRankingRepository.findAll();
        }
    ...
    public PlayerRankingDTO populatePlayerMMR(Long playerId, CreatePlayerRequest createPlayerRequest) {
        ...
        Long queueId = createPlayerRequest.queueId()
        ...
        int initialMmr = switch (queue.getMmrRuleType()) {
