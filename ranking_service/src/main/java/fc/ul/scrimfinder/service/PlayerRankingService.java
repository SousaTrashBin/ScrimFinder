...
public interface PlayerRankingService {
    List<PlayerRankingDTO> getPlayerRanking(Long playerId, Optional<Long> queueId) throws PlayerNotFoundException, QueueNotFoundException;

    Map<Long, PlayerRankingDTO> processMatchResults(@Valid MatchResultRequest matchResultRequest) throws PlayerNotFoundException, QueueNotFoundException;

    PaginatedResponseDTO<PlayerRankingDTO> getQueueLeaderboard(int page, int size, Optional<Long> queueId);

    PlayerRankingDTO populatePlayerMMR(Long playerId, @Valid CreatePlayerRequest createPlayerRequest) throws MMRAlreadyExistsException, QueueNotFoundException, LeagueAccountNotLinkedException;
}
