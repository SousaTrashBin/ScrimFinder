...
public class QueueController {
    ...
    @APIResponses(value = {
            @APIResponse(responseCode = "201", description = "Queue created successfully",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = QueueDTO.class))),
            @APIResponse(responseCode = "409", description = "Queue ID already exists",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponse.class)))
    })
    public Response createQueue(@PathParam("id") Long id,
        ...
        QueueDTO queue = queueService.createQueue(id, name, namespace, requiredPlayers, isRoleQueue, mode, mmrWindow);
