...
public class RiotMapper {
    public static MatchStatsDTO toMatchStatsDTO(JsonNode match) {
        String riotMatchId = new JsonNodeFinder(match)
                .jsonGetOrThrow("metadata", InvalidMatchFormatException.class)
                .jsonGetOrThrow("matchId", InvalidMatchFormatException.class)
                .jsonNode().asText();

        JsonNodeFinder jsonNodeFinder = new JsonNodeFinder(match).jsonGetOrThrow("info", InvalidMatchFormatException.class);

        Long queueId = jsonNodeFinder
                .jsonGetOrThrow("queueId", InvalidMatchFormatException.class)
                .jsonNode().asLong();

        String gameVersion = jsonNodeFinder
        ...
        if (gameVersionElements.length < 2) {
            throw new InvalidMatchFormatException("Not enough elements in gameVersion field of Riot response: " + gameVersion);
        }
        String patch = String.format("%s.%s", gameVersionElements[0], gameVersionElements[1]);

        long gameCreationMillis = jsonNodeFinder
                .jsonGetOrThrow("gameStartTimestamp", InvalidMatchFormatException.class)
                .jsonNode().asLong();
        LocalDateTime gameCreation = LocalDateTime.ofInstant(Instant.ofEpochMilli(gameCreationMillis), ZoneId.systemDefault());

        Long gameDuration = jsonNodeFinder
                .jsonGetOrThrow("gameDuration", InvalidMatchFormatException.class)
                .jsonNode().asLong();

        JsonNode participants = jsonNodeFinder
                .jsonGetOrThrow("participants", InvalidMatchFormatException.class)
                .jsonNode();
        List<PlayerStatsDTO> players = StreamSupport.stream(participants.spliterator(), true)
                .map(RiotMapper::toPlayerStatsDTO)
                .toList();

        JsonNode teamsNode = jsonNodeFinder
                .jsonGetOrThrow("teams", InvalidMatchFormatException.class)
                .jsonNode();
        List<TeamStatsDTO> teams = StreamSupport.stream(teamsNode.spliterator(), true)
    ...
    public static TeamStatsDTO toTeamStatsDTO(JsonNode team, List<PlayerStatsDTO> players) {
        ...
        TeamSide side = switch (teamId) {
            ...
            case 200 -> TeamSide.RED;
            default ->
                    throw new InvalidTeamFormatException("Invalid team id from Riot response. Should be 100 or 200. Got: " + teamId);
        };

        List<PlayerStatsDTO> teamPlayers = players.stream()
                .filter(player -> player.side().equals(side))
                .toList();

        Integer teamKills = teamPlayers.stream()
                .map(PlayerStatsDTO::kills)
                .reduce(0, Integer::sum);
        Integer teamDeaths = teamPlayers.stream()
                .map(PlayerStatsDTO::deaths)
                .reduce(0, Integer::sum);
        Integer teamAssists = teamPlayers.stream()
                .map(PlayerStatsDTO::assists)
                .reduce(0, Integer::sum);
        Integer teamHealing = teamPlayers.stream()
                .map(PlayerStatsDTO::healing)
                .reduce(0, Integer::sum);
