...
RIOT_MATCH_IDS = ["EUW1_7824036381"]
...
def create_and_link_players(queue_id, run_id):
    ...
    for name, tag in RIOT_ACCOUNTS:
        p_id = str(
            uuid.uuid5(uuid.NAMESPACE_DNS, name.lower().replace(" ", "") + run_id)
        )
...
def run_pipeline():
    ...
    print(
        f"--- Starting FINAL E2E Matchmaking & History Pipeline Test (Run: {run_id}) ---"
    )
    ...
    for p in players:
        try:
            resp = requests.post(
                f"{MATCHMAKING_URL}/tickets",
                json={"playerId": p["id"], "queueId": queue_id},
            )
            resp.raise_for_status()
    ...
    try:
        requests.post(
            f"{MATCHMAKING_URL}/tickets/matches/{match_id}/complete"
        ).raise_for_status()
    ...
    print("[8/8] Verifying cross-service state changes...")
    time.sleep(15)
    ...
    try:
        ...
        if hist_resp.status_code == 200:
            ...
            print(
                f"  - WARNING: Match not found in History Service (Status {hist_resp.status_code})."
            )
    except Exception as e:
    ...
    try:
        ...
        if rank_resp.status_code == 200:
            ...
            if rankings and rankings[0]["mmr"] != 1000:
                print(
                    f"  - SUCCESS: Ranking Service updated MMR (New: {rankings[0]['mmr']})."
                )
            else:
                print("  - WARNING: MMR remains at 1000.")
        else:
            print(
                f"  - ERROR: Ranking verification failed Status: {rank_resp.status_code}"
            )
    except Exception as e:
