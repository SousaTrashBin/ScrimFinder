...
helm upgrade --install scrimfinder "$ROOT_DIR/k8s/charts/scrimfinder" \
  ...
  --set detailFillingExternal.enabled=false \
  ...
  --set services.detail-filling-service.enabled=true \
  --set services.ranking-service.env.DETAIL_FILLING_SERVICE_URL="http://detail-filling-service:8080/api/v1/riot" \
  --set services.match-history-service.env.PLAYER_FILLING_SVC_URL="http://detail-filling-service:8080/api/v1/riot" \
  --set services.training-service.env.DETAIL_FILLING_URL="http://detail-filling-service:8080/api/v1/riot"
...
kubectl rollout status deployment/detail-filling-service -n "$SCRIM_NAMESPACE" --timeout=10m
