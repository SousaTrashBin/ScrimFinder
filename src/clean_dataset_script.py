...
def main():
    ...
    if TEST_MODE: riot_files, pro_files = riot_files[:100], pro_files[:50]

    results = {'matches': [], 'team_stats': [], 'bans': [], 'player_stats': [], 'items': [], 'runes': [],
               'pro_players': [], 'pro_teams': [], 'tourney_matches': [],
               'dim_champs': {}, 'dim_players': {}, 'dim_items': {}, 'dim_runes': {}}

    with Pool(cpu_count()) as pool:
        for res in tqdm(pool.imap_unordered(parse_riot_file, riot_files), total=len(riot_files), desc="Parsing Ranked"):
            if res:
                results['matches'].append(res[0]); results['team_stats'].extend(res[1])
                results['bans'].extend(res[2]); results['player_stats'].extend(res[3])
                results['items'].extend(res[4]); results['runes'].extend(res[5])
        ...
        for res in tqdm(pool.imap_unordered(parse_pro_file, pro_files), total=len(pro_files), desc="Parsing Esports"):
            if res:
                results['matches'].append(res[0]); results['team_stats'].extend(res[1])
                results['bans'].extend(res[2]); results['player_stats'].extend(res[3])
                results['items'].extend(res[4])
                # MISSING LINES ADDED HERE:
                results['runes'].extend(res[5]) 
                results['dim_champs'].update(res[6])
                results['dim_items'].update(res[8])
                results['dim_runes'].update(res[9])
                results['pro_players'].extend(res[10]); results['pro_teams'].extend(res[11])
                results['tourney_matches'].extend(res[12])

    print("\nSaving finalized CSVs...")
    for key in ['matches', 'team_stats', 'bans', 'player_stats', 'items', 'runes', 'tourney_matches', 'pro_players', 'pro_teams']:
        df = pd.DataFrame(results[key])
        if not df.empty:
            df.drop_duplicates().to_csv(f"{OUTPUT_DIR}/{key}.csv", index=False)
    ...
    for data_dict, filename in dim_configs:
        ...
        if not df.empty: df.drop_duplicates(subset=['id']).to_csv(f"{OUTPUT_DIR}/{filename}.csv", index=False)
