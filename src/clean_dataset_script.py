...
CHAMP_ID_TO_NAME, CHAMP_NAME_TO_ID, ITEM_NAME_MAP, RUNE_NAME_MAP, SPELL_NAME_MAP = (
    get_static_data()
)
...
def parse_riot_file(file_path):
    try:
        ...
        if not match_id or info.get("gameMode") != "CLASSIC":
            return None
        ...
        for p in info.get("participants", []):
            ...
            p_stats.append(
                {
                    ...
                    "dmg_champs": p.get("totalDamageDealtToChampions"),
            ...
            for style in p.get("perks", {}).get("styles", []):
                for sel in style.get("selections", []):
                    ...
                    p_runes.append(
                        {"match_id": match_id, "puuid": puuid, "rune_id": rid}
                    )
...
def main():
    ...
    with Pool(cpu_count()) as pool:
        for i, res in enumerate(
            tqdm(
                pool.imap_unordered(parse_riot_file, riot_files, chunksize=50),
                total=len(riot_files),
                desc="Ranked",
            )
        ):
            if res:
                batch_data["matches"].append(res[0])
                batch_data["team_stats"].extend(res[1])
                batch_data["bans"].extend(res[2])
                batch_data["player_stats"].extend(res[3])
                batch_data["items"].extend(res[4])
                batch_data["runes"].extend(res[5])
                results["dim_champs"].update(res[6])
                results["dim_players"].update(res[7])
    ...
    dim_configs = [
        (results["dim_champs"], "dim_champions"),
        (results["dim_items"], "dim_items"),
        (results["dim_runes"], "dim_runes"),
    ]
