    ...
    try:
        ...
        for team in info.get("teams", []):
            ...
            for b in team.get("bans", []):
                ...
                if cid != "-1":
                    bans.append(
                        {"match_id": match_id, "team_id": t_id, "champion_id": cid, "pick_turn": b.get("pickTurn")}
                    )
...
def main():
    ...
    with Pool(cpu_count()) as pool:
        for i, res in enumerate(
            tqdm(pool.imap_unordered(parse_riot_file, riot_files, chunksize=50), total=len(riot_files), desc="Ranked")
        ):
            if res:
