...
"""
import os, tempfile, time, pytest
...
class TestGames:
    ...
    def test_get_404(self):    assert client.get("/games/NOPE_XYZ").status_code == 404
    def test_list(self):
        r = client.get("/games")
        assert r.status_code == 200
...
class TestFeatures:
    ...
    def test_get_after_store(self):
        ...
        assert r.status_code == 200
    def test_get_404(self): assert client.get("/features/NOPE_XYZ").status_code == 404
...
class TestModels:
    def test_list(self):    assert "models" in client.get("/models").json()
    def test_active(self):
        for m in client.get("/models/active").json()["models"]: assert m["is_active"] is True
