...
from training_service.core.schemas import (
    DatasetCreateRequest, DatasetBuildRequest, DatasetMeta, DatasetListResponse, ErrorResponse)

router = APIRouter(prefix="/datasets", tags=["Datasets"])

def _meta(r):
    return DatasetMeta(id=r["id"], name=r["name"], description=r.get("description",""),
        concern=r["concern"], filters=r["filters"], game_count=r["game_count"],
        row_count=r["row_count"], status=r["status"], created_at=r["created_at"],
        built_at=r.get("built_at"), file_path=r.get("file_path"))
...
@router.post("", response_model=DatasetMeta, status_code=201, summary="Register dataset")
def create_dataset(body: DatasetCreateRequest):
    ds_id = "ds_" + uuid.uuid4().hex[:12]
    db.insert_dataset(ds_id, body.name, body.concern.value,
                      body.filters.model_dump(exclude_none=True), body.description)
    return _meta(db.get_dataset(ds_id))

@router.post("/build", response_model=DatasetMeta, status_code=202, summary="Create and build dataset")
...
@router.post("/{dataset_id}/build", response_model=DatasetMeta, status_code=202,
             summary="Rebuild dataset", responses={404: {"model": ErrorResponse}})
...
def delete_dataset(dataset_id: str = Path(...)):
    ...
    if active:
        raise HTTPException(status_code=409, detail=f"Dataset has {active} active job(s). Cancel them first.")
