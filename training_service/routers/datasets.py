...
def create_dataset(body: DatasetCreateRequest):
    ...
    db.insert_dataset(
        ds_id, body.name, body.concern.value, body.filters.model_dump(exclude_none=True), body.description
    )
...
@router.get(
    "/{dataset_id}", response_model=DatasetMeta, summary="Get dataset", responses={404: {"model": ErrorResponse}}
)
...
def delete_dataset(dataset_id: str = Path(...)):
    ...
    with db.get_conn() as conn:
        active = conn.execute(
            "SELECT COUNT(*) FROM training_jobs WHERE dataset_id=? AND status IN ('PENDING','RUNNING')", (dataset_id,)
        ).fetchone()[0]
    if active:
