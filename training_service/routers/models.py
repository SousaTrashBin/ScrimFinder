...
from training_service.core.schemas import ModelMeta, ModelListResponse, ErrorResponse

router = APIRouter(prefix="/models", tags=["Models"])

def _meta(r):
    return ModelMeta(id=r["id"], concern=r["concern"], algorithm=r["algorithm"],
        dataset_id=r.get("dataset_id"), version=r["version"],
        metrics=r.get("metrics") or {}, hyperparams=r.get("hyperparams") or {},
...
@router.get("/{model_id}", response_model=ModelMeta, summary="Get model metadata",
...
@router.post("/{model_id}/activate", response_model=ModelMeta, summary="Activate a model",
             responses={404: {"model": ErrorResponse}})
...
@router.post("/{model_id}/deactivate", response_model=ModelMeta, summary="Deactivate a model",
             responses={404: {"model": ErrorResponse}})
