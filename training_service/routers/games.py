...
from training_service.core.schemas import (
    GameIngest, GameIngested, GameDetail, GameListResponse,
...
def _row(r): return GameIngested(id=r["id"], source=r["source"], patch=r["patch"],
    match_type=r["match_type"], ingested_at=r["ingested_at"])
...
def ingest_batch(body: BatchIngestRequest):
    ...
    for item in body.games:
        try:
            ...
            errors.append({"id": item.id, "error": str(e)}); skipped += 1
...
def list_games(source: Optional[str]=None, patch: Optional[str]=None,
    ...
    rows, total = db.list_games(source=source, patch=patch, match_type=match_type,
                                limit=limit, offset=offset)
    return GameListResponse(games=[_row(r) for r in rows],
                            meta=PaginatedMeta(total=total, limit=limit, offset=offset))

@router.get("/{game_id}", response_model=GameDetail, summary="Fetch stored match",
            responses={404: {"model": ErrorResponse}})
def get_game(game_id: str = Path(...)):
    row = db.get_game(game_id)
    if row is None:
        raise HTTPException(status_code=404, detail=f"Game '{game_id}' not found.")
    return GameDetail(id=row["id"], source=row["source"], patch=row["patch"],
        match_type=row["match_type"], duration_sec=row.get("duration_sec"),
        platform=row.get("platform"), ingested_at=row["ingested_at"], raw_json=row["raw_json"])

@router.delete("/{game_id}", status_code=204, summary="Delete a game",
               responses={404: {"model": ErrorResponse}})
