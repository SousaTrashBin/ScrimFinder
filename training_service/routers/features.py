...
from training_service.core.schemas import (
    FeatureExtractRequest, FeatureExtractResponse, FeatureVector, ErrorResponse, Concern)

router = APIRouter(prefix="/features", tags=["Features"])

@router.post("/extract", response_model=FeatureExtractResponse, summary="Extract feature vectors",
...
def extract_features(body: FeatureExtractRequest):
    ...
    return FeatureExtractResponse(game_id=game_id, features=vectors,
                                  stored=body.store and body.game_id is not None)

@router.get("/{game_id}", response_model=list[FeatureVector], summary="Get cached features",
            responses={404: {"model": ErrorResponse}})
def get_features(game_id: str = Path(...), concern: Optional[Concern] = Query(None)):
    ...
    return [FeatureVector(game_id=r["game_id"], concern=r["concern"],
        feature_vector=r["feature_vector"], feature_names=r["feature_names"],
        schema_version=r["schema_version"], extracted_at=r["extracted_at"]) for r in rows]

@router.delete("/{game_id}", status_code=204, summary="Delete cached features",
               responses={404: {"model": ErrorResponse}})
