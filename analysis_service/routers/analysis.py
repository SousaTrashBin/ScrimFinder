...
def _load_artifact(concern: str) -> tuple:
    ...
    if not path or not os.path.exists(path):
        raise HTTPException(
            status_code=503,
            detail=f"Model file not found at '{path}'. Re-train to regenerate."
        )
...
def _percentile_score(value: float, percentile_data: dict, metric: str) -> float:
    ...
    if value <= p.get("p10", 0):   return 10.0
    if value <= p.get("p25", 0):   return 25.0
    if value <= p.get("p50", 0):   return 50.0
    if value <= p.get("p75", 0):   return 75.0
    if value <= p.get("p90", 0):   return 90.0
    return 95.0


def _score_player(pipe, pos_le, champ_le, position_db: str, champion_id: int,
                  stats: dict) -> Optional[float]:
    ...
    try:
        ...
        numeric   = np.array([[
            ...
            stats.get("gold", 0),  stats.get("cs", 0),     stats.get("dmg_champs", 0),
            stats.get("vision", 0), stats.get("kda", 0),   stats.get("kp", 0),
        ]], dtype=np.float32)
    ...
    for stat, metric, threshold_pct, category, tip_text, impact in checks:
        ...
        if pct < threshold_pct:
            tips.append(ImprovementTip(
                category=category, tip=tip_text,
                impact=impact, related_metric=metric,
            ))
        if len(tips) >= 3:
            break
...
def analyze_draft(body: DraftAnalysisRequest) -> DraftAnalysisResponse:
    ...
    try:
        ...
        def resolve(team) -> list[int]:
            return [cid for c in team.champions
                    if (cid := get_champion_id(c.name)) is not None]

        blue_ids = resolve(body.team_blue)
        red_ids  = resolve(body.team_red)

        if len(blue_ids) == 5 and len(red_ids) == 5:
            X        = np.hstack([mlb.transform([blue_ids]),
                                   mlb.transform([red_ids])]).astype(np.float32)
    ...
    try:
        ...
        if cid is not None:
            ...
            try:
                ...
                X         = np.hstack([item_enc, rune_enc, pos_enc,
                                        champ_enc, numeric]).astype(np.float32)
...
def analyze_player(body: PlayerAnalysisRequest) -> PlayerAnalysisResponse:
    ...
    if not stats_rows:
        return PlayerAnalysisResponse(
            ...
            role=body.role.value if body.role else None,
            win_rate=0.0,
            kda=None, avg_damage=None, avg_vision=None,
            avg_gold_pm=None, obj_participation=None,
            tips=[ImprovementTip(
                category=TipCategory.GAME_SENSE,
...
def analyze_game(body: GameAnalysisRequest) -> GameAnalysisResponse:
    ...
    for p in participants:
        ...
        if pipe and pos_le and champ_le:
            perf_score = _score_player(
                pipe, pos_le, champ_le,
                position, champ_id,
                {"kills": kills, "deaths": deaths, "assists": assists,
                 "gold": gold, "cs": cs, "dmg_champs": dmg,
                 "vision": vision, "kda": kda, "kp": kp},
            )

        highlights, lowlights = _highlight_player(kills, deaths, cs, vision, kda)

        players.append(PlayerOutcome(
            puuid=puuid,
            champion=champ_name,
    ...
    return GameAnalysisResponse(
        ...
        players=players,
        team_synergies={
            "blue": _synergies([p for p in players
                                 if raw.get("participants", [{}])[players.index(p)].get("teamId") == 100]),
            "red":  [],
        },
...
def analyze_champion(body: ChampionAnalysisRequest) -> ChampionAnalysisResponse:
    from analysis_service.champion.queries import (
        get_champion_id, query_winrate, query_stats,
        query_top_items, query_counters,
    )
    ...
    try:
        ...
        if cid is None:
            raise HTTPException(
                status_code=404,
                detail=f"Champion '{body.champion}' not found in EUW dataset."
            )
    ...
    tier     = ("S" if win_rate >= 53 else "A" if win_rate >= 51
                else "B" if win_rate >= 49 else "C" if win_rate >= 47 else "D")
...
def _eval_build(items: list[str], enemies: Optional[list[str]]) -> tuple[list[str], list[str]]:
    s, w    = [], []
    item_set = set(items)
    if "Kraken Slayer"     in item_set: s.append("Strong tank-shredding capability")
    if "Infinity Edge"     in item_set: s.append("High critical strike damage ceiling")
    if "Runaan's Hurricane" in item_set: s.append("Multi-target AoE for teamfights")
    if "Rabadon's Deathcap" in item_set: s.append("Massive AP amplification")
    if "Zhonya's Hourglass" in item_set: s.append("Invulnerability to survive burst")
...
def _highlight_player(kills: int, deaths: int, cs: int,
                       vision: int, kda: float) -> tuple[list[str], list[str]]:
    highlights, lowlights = [], []
    if kills >= 10:        highlights.append(f"Outstanding {kills} kills")
    if deaths == 0:        highlights.append("Perfect deathless game")
    if cs >= 250:          highlights.append(f"Excellent {cs} CS")
    if vision >= 40:       highlights.append(f"Strong vision control ({vision})")
    if kda >= 5:           highlights.append(f"Exceptional KDA of {kda}")
    if deaths >= 10:       lowlights.append(f"Died {deaths} times — positioning needs work")
    if cs < 100:           lowlights.append(f"Only {cs} CS — focus on farm")
    if vision < 10:        lowlights.append("Low vision score — buy more wards")
